...
/**
 * A Gene implementation that supports an integer values for its allele. Upper and lower bounds may optionally be
 * provided to restrict the range of legal values allowed by this Gene instance.
 *
 * @author Neil Rotstan
 * @author Klaus Meffert
 * @since 1.0
 */
public class LongGene extends NumberGene implements IPersistentRepresentation {

    /**
     *
     */
    private static final long   serialVersionUID = 7637504345823455179L;
    ...
    /**
     * The upper bounds of values represented by this Gene. If not explicitly provided by the user, this should be set
     * to Integer.MAX_VALUE.
     */
    private int                 m_upperBounds;

    /**
     * The lower bounds of values represented by this Gene. If not explicitly provided by the user, this should be set
     * to Integer.MIN_VALUE
     */
    private int                 m_lowerBounds;

    /**
     * Constructs a new IntegerGene with default settings. No bounds will be put into effect for values (alleles) of
     * this Gene instance, other than the standard range of integer values.
     * <p>
     ...
     * @since 1.0
     */
    public LongGene() throws InvalidConfigurationException{
     ...
     * @since 3.0
     */
    public LongGene(final Configuration a_config) throws InvalidConfigurationException{
        this(a_config, Integer.MIN_VALUE, Integer.MAX_VALUE);
    }

    /**
     * Constructs a new IntegerGene with the specified lower and upper bounds for values (alleles) of this Gene
     * instance.
     *
     * @param a_config the configuration to use
     ...
     * @since 2.0
     */
    public LongGene(final Configuration a_config, final int a_lowerBounds, final int a_upperBounds)
                                                                                                   throws InvalidConfigurationException{
        super(a_config);
    ...
    protected Gene newGeneInternal() {
        try {
            LongGene result = new LongGene(getConfiguration(), m_lowerBounds, m_upperBounds);
            return result;
    ...
    /**
     * Retrieves a string representation of this Gene that includes any information required to reconstruct it at a
     * later time, such as its value and internal state. This string will be used to represent this Gene in XML
     * persistence. This is an optional method but, if not implemented, XML persistence and possibly other features will
     * not be available. An UnsupportedOperationException should be thrown if no implementation is provided.
     *
    ...
    public String getPersistentRepresentation() {
        ...
        return s + PERSISTENT_FIELD_DELIMITER + m_lowerBounds + PERSISTENT_FIELD_DELIMITER + m_upperBounds;
    }
     ...
     *
     * @param a_representation the string representation retrieved from a prior call to the
     * getPersistentRepresentation() method
     * @throws UnsupportedOperationException to indicate that no implementation is provided for this method
     * @throws UnsupportedRepresentationException if this Gene implementation does not support the given string
     * representation
     * @author Neil Rostan
     * @since 1.0
     */
    public void setValueFromPersistentRepresentation(final String a_representation)
        ...
        if (a_representation != null) {
            ...
            if (tokenizer.countTokens() != 3) {
                throw new UnsupportedRepresentationException("The format of the given persistent representation "
    ...
    /**
     * Retrieves the int value of this Gene, which may be more convenient in some cases than the more general
     * getAllele() method.
     *
    ...
    /**
     * Sets the value (allele) of this Gene to a random Integer value between the lower and upper bounds (if any) of
     * this Gene.
     *
     * @param a_numberGenerator the random number generator that should be used to create any random values. It's
     * important to use this generator to maintain the user's flexibility to configure the genetic engine to use the
    ...
    public void setToRandomValue(final RandomGenerator a_numberGenerator) {
        double randomValue = ((long) m_upperBounds - (long) m_lowerBounds) * a_numberGenerator.nextDouble()
                             + m_lowerBounds;
        setAllele(new Long((int) Math.round(randomValue)));
    }

    /**
     * Compares to objects by first casting them into their expected type (e.g. Integer for IntegerGene) and then
     * calling the compareTo-method of the casted type.
     *
     ...
     * the object provided for comparison
     * @author Neil Rostan
     * @since 1.0
     */
    ...
    /**
     * Maps the value of this IntegerGene to within the bounds specified by the m_upperBounds and m_lowerBounds instance
     * variables. The value's relative position within the integer range will be preserved within the bounds range (in
     * other words, if the value is about halfway between the integer max and min, then the resulting value will be
     * about halfway between the upper bounds and lower bounds). If the value is null or is already within the bounds,
    ...
    protected void mapValueToWithinBounds() {
        if (getAllele() != null) {
            ...
            // -----------------------------------------------------------------
            if (i_value.longValue() > m_upperBounds || i_value.longValue() < m_lowerBounds) {
                RandomGenerator rn;
                ...
                if (m_upperBounds == m_lowerBounds) {
                    setAllele(new Long(m_lowerBounds));
                } else {
                    setToRandomValue(rn);
                }
            }
     ...
     * @param a_percentage percentage of mutation (greater than -1 and smaller than 1)
     * @author Klaus Meffert
     * @author David Kemp
     * @since 1.1
     */
    public void applyMutation(final int a_index, final double a_percentage) {
        ...
        if (getAllele() == null) {
            setAllele(new Long((int) range + m_lowerBounds));
        } else {
            int newValue = (int) Math.round(longValue() + range);
    ...
    /**
     * @return string representation of this Gene's value that may be useful for display purposes
     * @author Klaus Meffert
     * @since 2.4
     */
