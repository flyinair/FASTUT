 ...
 * A file called "filename.lock" is created and Java's FileLock class is used to lock the file. The java.nio classes
 * were introduced in Java 1.4, so this class does a no-op when used with Java 1.3. The class maintains compatability
 * with Java 1.3 by accessing the java.nio classes using reflection.
 *
 * @author John Lewis
 * @author Mark Doliner
 */
public class FileLocker {
     ...
     */
    public boolean lock() {
        String useNioProperty = System.getProperty("fastut.use.java.nio");
     ...
     */
    public void release() {
        if (lock != null) lock = releaseFileLock(lock);

        if (lockChannel != null) lockChannel = closeChannel(lockChannel);
        if (!lockFile.delete()) {
